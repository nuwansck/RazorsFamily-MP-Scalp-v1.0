"""Signal engine for EMA crossover + ORB scalping — RF Scalp v1.0 Multipair

Supports GBP_USD, EUR_USD, GBP_JPY, USD_JPY (and any other OANDA forex pair).
...
Key per-pair settings used here: sl_pct, tp_pct, rr_ratio, tp_mode, pip_size.
...
def _price_dp(pip_size: float) -> int:
    ...
    if pip_size <= 0.0001:
        return 5   # GBP_USD, EUR_USD (e.g. 1.27345)
    if pip_size <= 0.01:
        return 3   # GBP_JPY, USD_JPY (e.g. 190.234)
    return 2       # fallback
...
class SignalEngine:
    ...
    def analyze(self, instrument: str = "GBP_USD", settings: dict | None = None):
        ...
        Args:
            instrument: OANDA instrument code (e.g. GBP_USD, EUR_USD, GBP_JPY, USD_JPY)
    ...
    def _get_orb(self, session_name, instrument: str, now_sgt: _dt,
        ...
        # Include instrument so GBP_USD and EUR_USD have separate ORBs
